...
# Bootstrap analysis
np.random.seed(42)
boot = tna.bootstrap_tna(df.head(500), n_boot=200, ci=0.95, seed=42)

print("Bootstrap Analysis Results:")
print(f"  Number of replicates: {boot.n_boot}")
print(f"  Confidence level: {boot.ci_level * 100}%")

# Summary of top edges
summary = boot.summary()
print("\nTop 5 edges by weight (with 95% CI):")
top_edges = summary.nlargest(5, 'estimate')[['from', 'to', 'estimate', 'ci_lower', 'ci_upper']]
print(top_edges.to_string(index=False))

# Significant edges
sig_edges = boot.significant_edges(threshold=0.1)
print(f"\nEdges significantly > 0.1: {len(sig_edges)}")
```
...
print(f"  Observed statistic: {perm_result.observed:.4f}")
print(f"  P-value: {perm_result.p_value:.4f}")
print(f"  Significant at alpha=0.05: {perm_result.is_significant(0.05)}")
...
print(f"  Significant (FDR < 0.05): {edge_tests['significant'].sum()}")
```
...
plt.suptitle('Bootstrap Analysis: Edge Weight Confidence Intervals',
...
```{python py-permutation-plot, fig.width=8, fig.height=6}
# Plot permutation test null distribution
ax = tna.plot_permutation(perm_result)
plt.title('Permutation Test: Null Distribution', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
```
